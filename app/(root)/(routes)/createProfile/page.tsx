...
const Page = () => {
  ...
  useEffect(() => {
    if (selectedCountry) {
      axios
        .get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/country_category/${selectedCountry}`
        )
        .then((response) => {
      ...
      axios
        .get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/template_master`)
        .then((response) => {
          //console.log("CREATE PROFILE COUNTRY DATA\n", response.data);
          // Check if the response contains the expected properties
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/template_details`,
        postFileData
      )
      .then((response) => {
